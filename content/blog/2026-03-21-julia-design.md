...
Julia의 Python daemon은 **이벤트를 수신하고 Claude Code를 실행하는 것이 전부**다. 판단, 계획, 도구 선택, 응답 생성은 모두 Claude Code 안에서 일어난다.
...
사실을 복사해서 저장하지 않고, 매 요청마다 원본을 조회한다 (Live query over cache).

## 3. 메모리는 합성된 지식만

Claude Code의 memory 디렉토리는 **합성된 지식**만 저장한다. 여러 소스를 종합한 판단, 사람과 프로젝트 사이의 연결, 사용자 선호도처럼 어떤 단일 시스템에서도 조회할 수 없는 것들이다.

GitHub에서 `gh`로 조회 가능한 사실, Gmail에서 읽을 수 있는 내용은 memory에 쓰지 않는다.

## 4. AI의 머릿속을 사람이 볼 수 있게

memory 파일을 Obsidian 호환 포맷으로 작성한다. frontmatter에 `tags`, `person`, `projects` 필드를 넣고, 본문에 `[[wikilinks]]`를 사용한다.

cron이 5분마다 memory를 Google Drive로 rsync하면, Mac의 Obsidian에서 AI의 knowledge graph를 탐색할 수 있다. Dataview 쿼리로 사람별, 프로젝트별 대시보드도 자동 생성된다.
...
이 규칙 덕분에 사용자는 요청이 처리되었는지 항상 확인할 수 있다.
...
Claude Code는 매번 실행될 때마다 이 파일을 로드한다. `CLAUDE.md`를 수정하면 코드 한 줄 안 건드리고 시스템의 행동이 바뀐다.

## 돌아보며

공통점은 **기존 도구를 최대한 활용하고, 새로 만드는 것을 최소화**한 것이다. GitHub, Google Workspace, Obsidian, rclone 위에 얇은 접착제(thin daemon + CLAUDE.md)만 올렸다. 개인용이라면 이 정도로 충분했다.
