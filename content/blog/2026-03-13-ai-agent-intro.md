...
ChatGPT나 Claude 같은 AI 챗봇은 이제 많이 쓰이고 있습니다. 최근에는 여기서 더 나아간 **AI Agent**가 확산되고 있습니다. 질문에 답하는 것뿐 아니라, 이메일을 보내고, 일정을 잡고, 문서를 정리하는 등 **실제 업무를 수행**하는 AI입니다.

이 글에서는 AI Agent가 무엇인지, 어떻게 시작하는지, 교육·행정 분야의 활용 사례를 정리합니다. 코딩 지식은 필요 없습니다.
...
핵심 차이는 **도구 사용 능력**입니다. AI Agent는 이메일, 캘린더, 파일 시스템 등에 직접 접근하여 작업을 완료합니다.
...
2. **민감한 정보에 주의하세요**: 학생 개인정보, 미발표 연구 데이터 등은 외부 AI 서비스에 올리기 전에 각 서비스의 데이터 정책을 확인하세요.

3. **결과를 검증하세요**: AI는 그럴듯하지만 틀린 답을 할 수 있습니다 (hallucination). 특히 숫자, 인용, 사실 관계는 반드시 확인하세요.

4. **반복 작업부터 시작하세요**: 매주 반복하는 단순 업무(이메일 정리, 회의록, 성적 입력 안내문 등)부터 맡겨보세요.
